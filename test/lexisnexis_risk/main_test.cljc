(ns lexisnexis_risk.main-test
  "Contract + behavioral test for the lexisnexis_risk-compat L4 actor (cljc port).
...
(deftest healthz
  (let [[body status] (m/healthz)]
    (is (= 200 status)) (is (= "lexisnexis_risk-compat" (:actor body))) (is (= (set m/entities) (set (:entities body))))))

#?(:clj (defn -main [& _]
          (let [{:keys [fail error]} (run-tests 'lexisnexis_risk.main-test)]
