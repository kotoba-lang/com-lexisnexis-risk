(ns lexisnexis_risk.main
  "Kotodama WASM entrypoint for the Lexisnexis_risk clean-room actor (L4) — Clojure port.
  ...
  facts (`lexisnexis_risk.<Entity>/<field>`); `*store*` is the in-memory materialization
  used by the contract test and by the WASM runtime before a live engine binds."
  (:require [clojure.string :as str]))

(def ns-prefix "lexisnexis_risk")
(def tier "L4")
...
(def entity-specs
  [{:entity "Account"      :plural "accounts"      :id-prefix "lexisnex_acc"
    :fields [:holderId :type :balance :currency :status] :required [:type :balance]
    :coerce {:balance :float}            :refs {}}
   {:entity "Loan"         :plural "loans"         :id-prefix "lexisnex_loa"
...
(defn emit-facts
  "EAVT facts for one record: {\"lexisnexis_risk.<Entity>/<field>\" v ...}. The datomic
...
(defn healthz [] [{:status "ok" :actor "lexisnexis_risk-compat" :tier tier :entities entities} 200])
...
(defn start! [] :lexisnexis_risk-compat/ready)
